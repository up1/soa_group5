        ...
        $.ajax({
                ...
                for(var i = 0; i < showtime.length; i++) {
                    if(showtime[i].cinema.group == 'SF') {
                        ...
                        if(sf_list.indexOf(cinema_id) == -1) {
                            sf_list.push(cinema_id);
                            $('#showtime').append(`<div id="${showtime[i].cinema.id}" class="ui divided items" style="padding:14px;"><h3 class="ui block header left floated">
                                                                   ${showtime[i].cinema.cinema_name_en}
                                                               </h3>
                                                               </div>`);
                            if(i == 0 || showtime[i].movie_id != showtime[i-1].movie_id || showtime[i].show_theatre != showtime[i-1].show_theatre) {
                                $('#'+showtime[i].cinema.id).append(`<div class="item">
                                                                        <div class="head">
                                                                        ...
                                                                        <div class="content">
                                                                            ...
                                                                            <div id="${showtime[i].show_theatre}" class="description">
                                                                                <button class="ui red basic button">${showtime[i].show_time}</button>
                                                                            </div>
                                                                        </div>
                                                                    </div>`);
                            } else {
                                console.log("showtime = " + showtime[i].showtime_id);
                                $("#"+showtime[i].show_theatre).append(`<button class="ui red basic button">${showtime[i].show_time}</button>`);
                            }
                            ...
                            if(i == 0 || showtime[i].movie_id != showtime[i-1].movie_id || showtime[i].show_theatre != showtime[i-1].show_theatre) {
                                $('#'+showtime[i].cinema.id).append(`<div class="item">
                                                 ...
                                                 console.log("showtime = " + showtime[i].showtime_id);
                                                 $('#'+showtime[i].show_theatre+ ' .description').append(`<button class="ui red basic button">${showtime[i].show_time}</button>`);
                                             }
                        }
                    }
                }
